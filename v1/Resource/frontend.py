...
class Interface(Request):
    ...
    """
        ...
        '': ''
    """
    font_title = 'Microsoft_JhengHei 18 bold'
    font_text = 'Microsoft_JhengHei 12 bold'
    ...
    def show_login(self):
        ...
        loginId_label = tk.Label(login_window, text='帳號：', font=self.font_text)
        loginId_label.pack()
        loginId_entry = tk.Entry(login_window, width=30)
        loginId_entry.pack()
        v = tk.BooleanVar()
        v.set(False)
        password_label = tk.Label(login_window, text='密碼：', font=self.font_text)
        password_label.pack()
        password_entry = tk.Entry(login_window, show='*', width=30)
        password_entry.pack()
        password_check = tk.Checkbutton(login_window, text='顯示密碼', variable=v, height=2, command=check, font=self.font_text)
        password_check.pack()
        login_button = tk.Button(login_window, text='登錄', command=login, font=self.font_text)
        login_button.pack()
    ...
    def show_anno(self):
        ...
        if successful:
            ...
            anno_title_label = tk.Label(anno_window, text='公告', font=self.font_title)
            ...
            for i in range(len(announcement)):
                ...
                j += 2
            anno_label = tk.Label(anno_window, text=text, font=self.font_text)
            anno_label.pack()
...
def show_selection_window(inter: Interface):
    ...
    selection_window.rowconfigure((0, 1, 2, 3, 4), weight=1)
    ...
    login_button = tk.Button(selection_window, text='登陸', font=f, command=inter.show_validate_window,
                             width=16, height=2)
    login_button.grid(row=0, column=0, columnspan=2, sticky='n')
    ...
    delete_backup_button = tk.Button(selection_window, text='刪除備份', font=f, command=delete_backup,
                                     width=w, height=h)
    delete_backup_button.grid(row=4, column=1, sticky='nw')
    if_login = tk.StringVar()
    if_login.set('否')
    frame = tk.Frame(selection_window)
    if_login_label = tk.Label(frame, textvariable=if_login)
    if_login_label.pack()
    developer_label = tk.Label(frame, text='Developed by EFLin')
    developer_label.pack(side='bottom', anchor='e')
    frame.grid(column=1)
...
def show_login(inter: Interface):
    def create():
        ...
        inter.show_validate_window()
        f = inter.validate_login()
        if f:
            login_window.destroy()
